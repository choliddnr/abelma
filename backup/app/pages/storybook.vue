...
import type { LetterEntry } from '@/types/alphabet'
import InteractionZone from '@/components/InteractionZone.vue'

// ─── Data ────────────────────────────────────────────────────
const data = alphabetData as LetterEntry[]
const router = useRouter()
const store = useStorybookStore()

// ─── State ───────────────────────────────────────────────────
const { currentIndex, isSpeaking, challengeDone, showFinish, showChallenge, viewMode } = storeToRefs(store)
...
const buildPhonicsText = (entry: LetterEntry): string => {
...
watch([currentIndex, viewMode], ([newIdx, newMode], [oldIdx, oldMode]) => {
...
const onFail = () => { /* InteractionZone handles visual feedback */ }

// ─── Finish confetti ──────────────────────────────────────────
const launchFinishConfetti = () => {
  const end = Date.now() + 4000
  const frame = () => {
    confetti({ particleCount: 6, angle: 60, spread: 55, origin: { x: 0 }, colors: ['#FFD93D', '#6BCB77', '#4D96FF', '#ff9a9a', '#A084E8'], zIndex: 300 })
    confetti({ particleCount: 6, angle: 120, spread: 55, origin: { x: 1 }, colors: ['#FFD93D', '#6BCB77', '#4D96FF', '#ff9a9a', '#A084E8'], zIndex: 300 })
...
<template>
  ...
  <div class="min-h-screen transition-colors duration-700 ease-in-out flex flex-col" :class="viewMode === 'story' ? current.themeColor : 'bg-slate-50'">
    <!-- ── Top Bar ─────────────────────────────────────────── -->
    <header class="w-full max-w-5xl mx-auto flex items-center justify-between gap-3 p-4 md:p-6">
      <!-- Home/Back button -->
      <button v-if="viewMode === 'list'" @click="router.push('/')"
      ...
      <div v-if="viewMode === 'story'" class="flex flex-col items-center gap-1.5 flex-1">
        <h1 class="text-xl sm:text-3xl font-black text-white drop-shadow-md uppercase tracking-wide"
          style="font-family: 'Baloo Bhaijaan 2', sans-serif;">
          {{ current.title }}
        </h1>
        <!-- Progress dots -->
        <div class="flex flex-wrap justify-center gap-1">
          <button v-for="(entry, idx) in data" :key="entry.id" @click="goToLetter(idx)" :title="entry.letter.upper"
      ...
      <div v-else class="flex flex-col items-center gap-1.5 flex-1">
        <h1 class="text-xl sm:text-3xl font-black text-slate-700 drop-shadow-sm uppercase tracking-wide"
          style="font-family: 'Baloo Bhaijaan 2', sans-serif;">
          Buku Cerita Alfabet
        </h1>
      </div>
    ...
    <main class="flex-1 w-full max-w-5xl mx-auto px-4 pb-4 flex flex-col">
      <transition name="fade" mode="out-in">
        ...
        <!-- LIST VIEW -->
        <div v-if="viewMode === 'list'" key="list-view" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6 content-start pb-8">
          <button v-for="(entry, idx) in data" :key="entry.id" @click="goToLetter(idx)"
            ...
            :class="challengeDone.has(idx) ? 'border-green-400 border-2 border-b-8' : 'border-slate-200'">
            <div class="w-16 h-16 sm:w-20 sm:h-20 rounded-full flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform" :class="entry.themeColor">
              <span class="text-3xl sm:text-4xl font-black text-white drop-shadow-md" style="font-family: 'Baloo Bhaijaan 2', sans-serif;">{{ entry.letter.upper }}{{ entry.letter.lower }}</span>
            </div>
            <span class="text-sm font-bold text-slate-500 text-center leading-tight">{{ entry.title }}</span>
            <div v-if="challengeDone.has(idx)" class="absolute -top-3 -right-3 text-3xl drop-shadow-md animate-bounce">⭐</div>
          </button>
        </div>
        ...
        <div v-else key="story-view" class="flex flex-col flex-1 h-full w-full">
          <transition name="slide" mode="out-in">
            <div :key="currentIndex"
              ...
              <template v-if="!imageError">
                <img :src="storyImageSrc" :alt="current.title" class="absolute inset-0 w-full h-full object-cover"
                  @error="imageError = true" :key="'img-' + current.id" />
              ...
              <template v-else>
                <div class="absolute inset-0 bg-slate-50 flex items-center justify-center overflow-hidden">
                  <span class="text-[15rem] select-none opacity-[0.15]"
                    style="animation: letter-bounce 3s ease-in-out infinite;">{{ emojiMap[current.letter.upper] }}</span>
                </div>
              </template>
    
              <!-- Top-left overlay: Large Letter -->
              <div class="absolute top-6 left-6 md:left-8 flex items-end gap-2 drop-shadow-xl z-10 pointer-events-none">
                <span class="font-black leading-none select-none text-white drop-shadow-lg"
                  style="font-family: 'Baloo Bhaijaan 2', sans-serif; font-size: clamp(4rem, 8vw, 6rem); animation: letter-bounce 3s ease-in-out infinite;">
                  {{ current.letter.upper }}
                </span>
                <span class="font-black leading-none select-none text-white/90 drop-shadow-lg mb-1"
                  style="font-family: 'Baloo Bhaijaan 2', sans-serif; font-size: clamp(2.5rem, 5vw, 4rem);">
                ...
                <div
                  ...
                  <template v-if="!showChallenge">
                    ...
                    <div class="space-y-4 text-center">
                      ...
                      <p class="text-xl md:text-2xl lg:text-3xl font-medium leading-normal text-slate-700 max-w-2xl mx-auto"
                  ...
                  <template v-else>
                    ...
                    <div class="flex flex-col gap-6">
                      <div class="flex items-center justify-between w-full">
                        <button @click="showChallenge = false"
                          class="text-sm font-bold text-slate-400 hover:text-slate-600 transition-colors flex items-center gap-1">
                          <span>⬅️ Kembali</span>
                        </button>
                        <span class="text-sm font-black uppercase tracking-widest text-indigo-500">🎯 Tantangan</span>
                        <div class="w-16"></div> <!-- Spacer -->
                      </div>
    
                      <InteractionZone ref="interactionRef" :target="current.challenge.target"
                        :options="current.challenge.options" :instruction="current.challenge.instruction"
                        :themeColor="current.themeColor" @success="onSuccess" @fail="onFail" />
                    </div>
                  </template>
                </div>
              </div>
          ...
          <footer class="w-full mx-auto flex gap-4 pt-6 pb-2">
            <button @click="prevLetter" :disabled="currentIndex === 0"
              class="flex-1 bg-white/30 hover:bg-white/60 disabled:opacity-30 disabled:cursor-not-allowed
                     text-white hover:text-gray-700 py-4 rounded-3xl font-black text-lg transition-all
                     shadow-lg border-b-6 border-black/15 active:border-b-0 active:translate-y-1.5 flex items-center justify-center gap-2">
              ⬅️ <span class="hidden sm:inline">SEBELUMNYA</span>
            </button>
    
            <div class="flex flex-col items-center justify-center min-w-[4rem]">
              <span class="text-4xl font-black text-white drop-shadow-md"
                style="font-family: 'Baloo Bhaijaan 2', sans-serif;">
                {{ current.letter.upper }}
              </span>
              <span class="text-xs text-white/70 font-bold">{{ currentIndex + 1 }} / {{ data.length }}</span>
            </div>
    
            <button @click="nextLetter"
              class="flex-1 bg-green-400 hover:bg-green-300 text-white py-4 rounded-3xl font-black text-lg
                     transition-all shadow-lg border-b-6 border-green-600 active:border-b-0 active:translate-y-1.5 flex items-center justify-center gap-2">
    ...
    <transition name="pop-in">
      <div v-if="showFinish"
        ...
        <div class="glass-card p-8 sm:p-12 flex flex-col items-center gap-6 max-w-md w-full text-center">
          <span class="text-8xl animate-bounce">🎉</span>
          <h2 class="text-4xl font-black text-indigo-600" style="font-family: 'Baloo Bhaijaan 2', sans-serif;">
            Selamat!
          </h2>
