    ...
    :class="[
      animate
        ? variant === 'home'
          ...
          : 'animate-entrance'
        : '',
      variant === 'home' ? 'mb-8 md:mb-20' : 'my-4 md:my-6',
      props.class,
    ]"
  >
    ...
    <div v-if="variant === 'module'" class="flex justify-center mb-6">
      <div class="relative group">
        <!-- Outer Glow/Halo -->
        <div
          class="absolute -inset-1 bg-gradient-to-r from-white/30 to-white/10 rounded-full blur-xl opacity-50 group-hover:opacity-100 transition-opacity duration-700"
        ></div>

        <div
          class="relative flex flex-col items-center gap-4 bg-white/50 px-10 py-2 md:px-16 md:py-4 rounded-[3rem] border-4 border-white shadow-[0_20px_50px_rgba(0,0,0,0.1)] hover:shadow-[0_25px_60px_rgba(255,255,255,0.2)] transition-all duration-700 hover:-translate-y-2"
        >
          <!-- Icon Bubble -->
          <div class="inline-flex items-center gap-6">
            <div
              v-if="icon"
              class="flex items-center justify-center bg-white/50 size-12 md:size-16 rounded-full shadow-inner border-2 border-white/80"
            >
    ...
    <div v-else class="relative mb-12">
      <div
        class="absolute inset-0 bg-blue-400/10 blur-[100px] rounded-full scale-150"
      ></div>
      <h1
        class="relative text-4xl md:text-5xl lg:text-9xl font-black text-white drop-shadow-[0_5px_0_rgba(79,70,229,1)] font-quicksand title-gradient tracking-tight leading-none animate-float-slow"
      >
        {{ title }}
      </h1>
    </div>
...
@keyframes pulseSubtle {
  ...
  50% {
    box-shadow: 0 20px 80px rgba(79, 70, 229, 0.8);
  }
}
</style>
