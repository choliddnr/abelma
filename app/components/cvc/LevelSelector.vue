...
const isUnlocked = (levelId: string) => cvcProgress.value.unlockedLevels.includes(levelId);
const isPremiumLocked = (levelId: string) => !isPremium.value && levelId !== "3.1";
const getScore = (levelId: string) => cvcProgress.value.levelScores[levelId] || 0;

const getLink = (levelId: string) => {
  if (isPremiumLocked(levelId)) return "/subscription";
  return isUnlocked(levelId) ? `/cvc/play?level=${levelId}` : "#";
};
</script>

<template>
  <div class="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-5xl mx-auto p-4">
    <div
      v-for="level in cvcLevels"
      :key="level.id"
