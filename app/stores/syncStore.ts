    ...
    try {
      const syncData: Partial<CloudProfile>[] = profiles.map((p) => {
        ...
        return {
          ...
          name: p.name,
          avatar: p.avatar,
          coins: rewardStore.profileCoinsMap[id] || 0,
          ...
          stickers: (stickerStore.currentProfileStickers[id] || []).map(
            (sid: string) => ({
              stickerId: sid,
            }),
          ),
          analytics: Object.entries(analyticsStore.analyticsMap[id] || {}).map(
            ([tid, data]) => ({
              type: "word",
              targetId: tid,
  ...
  const syncAlphabet = async (retryCount = 0): Promise<void> => {
    ...
    try {
      ...
      const syncData: Partial<CloudProfile>[] = profiles.map((p) => {
        ...
        const storybook = (storybookStore.profileProgressMap[id] || [])
          .map((idx) => ({
            letter: alphabet[idx] || "",
            isCompleted: true,
      ...
      if (!response.ok)
        throw new Error(`Alphabet sync failed: ${response.statusText}`);
