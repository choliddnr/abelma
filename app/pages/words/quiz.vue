...
<template>
  <div class="flex flex-col min-h-screen relative overflow-hidden">
    ...
    <div
      v-if="isWin"
      class="absolute inset-0 z-100 flex flex-col gap-10 items-center justify-center bg-white/60 backdrop-blur-xl animate-in fade-in duration-1000"
    >
      <div class="relative animate-bounce">
        <h1
          class="text-7xl md:text-9xl font-black text-amber-500 drop-shadow-[0_12px_0_rgba(245,158,11,0.2)] font-quicksand"
        >
          LUAR BIASA!
        </h1>
        <div class="absolute -top-10 -right-10 text-6xl animate-float">🌟</div>
        <div
          class="absolute -bottom-10 -left-10 text-6xl animate-float"
      ...
      <div
        class="bg-linear-to-r from-amber-400 to-orange-500 text-white px-10 py-5 rounded-[2.5rem] font-black text-4xl shadow-2xl border-4 border-white animate-pop"
      >
        +{{ score }} KOIN! 🪙
      </div>
    ...
    <div
      v-if="showStickerModal && newSticker"
    ...
    >
      ...
      <div class="flex gap-2 md:gap-4">
        ...
        <div
          class="ui-capsule bg-indigo-500 border-indigo-600 text-white w-auto px-6 shadow-lg shadow-indigo-100 animate-float"
        >
          ⭐ {{ score }} / {{ maxScore }}
        </div>
      </div>
    ...
    >
      ...
      <div
        v-if="timerMax > 0"
        class="w-full max-w-md bg-white/50 h-4 rounded-full overflow-hidden border-2 border-white shadow-inner -mt-4"
      >
        <div
          class="h-full bg-linear-to-r from-emerald-400 to-sky-400 transition-all duration-100 ease-linear"
      ...
      >
        ...
        <button @click="playTargetAudio" class="relative group">
          ...
          >
            <span
              class="text-[8rem] md:text-[10rem] drop-shadow-md select-none group-hover:-rotate-6 transition-transform"
            >
              {{ currentTarget.emoji }}
            </span>
          </div>
          <!-- Replay Audio Hint -->
          <div
            class="absolute -bottom-2 -right-2 w-16 h-16 bg-blue-100 rounded-full shadow-lg border-4 border-white flex items-center justify-center opacity-80 group-hover:opacity-100 group-hover:scale-110 transition-all"
          >
          ...
          <div class="flex flex-wrap justify-center gap-2 md:gap-4 w-full">
            ...
            >
              ...
              >
                {{
                  settingsStore.settings.letterCase === "uppercase"
                    ? placedLetters[idx]?.toUpperCase()
                    : placedLetters[idx]?.toLowerCase()
                }}
              </span>
            </div>
          </div>
