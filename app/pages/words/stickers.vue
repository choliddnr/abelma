...
defineOptions({ name: "WordStickers" });
  ...
  <div class="min-h-screen bg-slate-50 flex flex-col p-4 md:p-8">
    ...
    <div class="flex-1 w-full max-w-5xl mx-auto">
      <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
        <div
          v-for="sticker in stickerStore.allAvailableStickers"
          :key="sticker.id"
          ...
          :class="
            stickerStore.earnedStickers.has(sticker.id)
        ...
        >
          <!-- Emoji Display -->
          <div
            class="text-6xl md:text-8xl mb-4 drop-shadow-md transition-transform duration-500"
            :class="
              stickerStore.earnedStickers.has(sticker.id) ? 'hover:scale-110 active:rotate-12' : ''
            "
          >
            {{ sticker.emoji }}
          </div>
          ...
          <div class="text-center">
            ...
            <p
              v-if="!stickerStore.earnedStickers.has(sticker.id)"
              class="text-xs text-slate-500 font-bold mt-1"
            >
              🔒 Butuh {{ sticker.requiredScore }} Poin
            </p>
            <p v-else class="text-xs text-emerald-500 font-black mt-1">✅ Dimiliki</p>
          </div>

          <!-- Shine Effect for Earned -->
          <div
            v-if="stickerStore.earnedStickers.has(sticker.id)"
            class="absolute top-0 inset-x-0 h-1/2 bg-linear-to-b from-white/40 to-transparent rounded-t-[inherit] pointer-events-none"
          ></div>
        </div>
      </div>
    </div>
