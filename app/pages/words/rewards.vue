...
const handleClaim = async (reward: Reward) => {
  if (reward.cost <= coins.value && reward.status === "available") {
...
<template>
  <div class="min-h-screen flex flex-col p-4 md:p-8 relative overflow-hidden">
    ...
    >
      ...
      <div
        class="ui-capsule bg-amber-400 border-amber-500 text-white w-auto px-6 animate-float shadow-lg shadow-amber-200"
      >
        <span class="text-xl md:text-2xl drop-shadow-md">🪙 {{ coins }}</span>
      </div>
    </div>
    ...
    <div class="relative z-10 flex-1 w-full max-w-5xl mx-auto">
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <div
          ...
          :class="[
            reward.status === 'available' && coins >= reward.cost
      ...
      >
        ...
        <h2 class="text-2xl font-black text-slate-400 font-quicksand">
          Belum ada hadiah yang disiapkan Ayah/Bunda.
        </h2>
      </div>
    </div>
