...
const isUnlocked = (levelId: number) => cvcProgress.value.learningLevel >= Number(levelId);
const isPremiumLocked = (levelId: number) => !isPremium.value && levelId !== 1;
const getScore = (levelId: number) => cvcProgress.value.levelScores[levelId] || 0;
...
<template>
  <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 w-full max-w-5xl mx-auto p-4">
    <div
