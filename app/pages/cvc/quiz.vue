...
const setupBuildBlend = () => {
  ...
  if (!isPremium.value) {
    allowedWords = cvcQuizWords.filter(
      (w) => w.answer === "H" || w.answer === "S",
    );
  }
  ...
  const allOptions = [target.answer, ...distractors].sort(
    () => Math.random() - 0.5,
  );
...
const handleCorrect = async () => {
  ...
  // Streak
  streak.value++;
  const { leveledUp } = await cvcStore.updateQuizScore(
    points,
...
<template>
  <div class="flex flex-col min-h-screen relative overflow-hidden">
    ...
    <!-- Question Area -->
    <main
      class="flex-1 flex flex-col items-center justify-center px-4 py-8 gap-8 z-10"
    >
      <Transition name="scale-fade" mode="out-in">
        <div
          :key="currentTarget?.id"
          class="w-full max-w-2xl flex flex-col items-center gap-10"
        >
          <!-- Activity: PICK_BLEND -->
          <div
            v-if="activityType === 'PICK_BLEND'"
            class="flex flex-col items-center gap-8 w-full"
          >
            <div
              class="size-48 md:size-64 bg-white rounded-[3rem] shadow-xl border-4 border-white flex items-center justify-center text-8xl md:text-[140px]"
            >
              {{ currentTarget?.emoji }}
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 w-full">
              <UiButton
                ...
                :class="[
                  wrongChoiceId === opt.id
                    ? 'bg-rose-500 border-rose-700 text-white shake'
                    : 'hover:border-indigo-200 text-slate-700',
                ]"
              >
                {{ opt.blend }}
              </UiButton>
          ...
          <div v-else class="flex flex-col items-center gap-8 w-full">
            <div class="flex items-center gap-4">
              <div
                class="size-24 md:size-32 bg-white rounded-3xl shadow-md border-2 border-white flex items-center justify-center text-5xl md:text-6xl"
              >
                {{ currentTarget?.emoji }}
              </div>
              <div
                class="flex items-center gap-2 bg-white/80 backdrop-blur-md p-6 rounded-[2.5rem] border-2 border-white shadow-xl"
              >
                <span class="text-5xl md:text-7xl font-black text-slate-400"
                  >{{ currentTarget?.prefix }}-</span
                >
                <span class="text-5xl md:text-7xl font-black text-slate-700">{{
                  currentTarget?.cv
                }}</span>
                <div
                  class="w-12 h-16 md:w-16 md:h-20 bg-slate-100 rounded-2xl border-4 border-dashed border-indigo-200 flex items-center justify-center"
                >
                  <span
                    v-if="
                      wrongChoiceId && wrongChoiceId === currentTarget?.answer
                    "
                    class="text-rose-500 text-5xl md:text-7xl font-black animate-ping"
                    >?</span
                  >
                </div>
              </div>
            ...
            <div class="grid grid-cols-4 gap-3 md:gap-4 w-full">
              <UiButton
                ...
                :class="[
                  wrongChoiceId === opt.id
                    ? 'bg-rose-500 border-rose-700 text-white shake'
                    : 'hover:border-indigo-200 text-indigo-600',
                ]"
              >
                {{ opt.text }}
              </UiButton>
