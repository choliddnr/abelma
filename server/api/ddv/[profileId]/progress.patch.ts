import { z } from "zod";
import { ddvProgress, profiles } from "#server/utils/db/schema";
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (!res) {
      throw createError({
        statusCode: 404,
        statusMessage: "DDV Progress not found",
      });
    }
    ...
    console.error("Error updating DDV progress:", error);
    throw createError({
      statusCode: 500,
      statusMessage: "Failed to update DDV progress",
    });
  }
});
