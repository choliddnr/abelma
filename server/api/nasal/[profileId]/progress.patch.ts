import { z } from "zod";
import { nasalProgress, profiles } from "#server/utils/db/schema";
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (!res) {
      throw createError({
        statusCode: 404,
        statusMessage: "Nasal Progress not found",
      });
    }
    ...
    console.error("Error updating Nasal progress:", error);
    throw createError({
      statusCode: 500,
      statusMessage: "Failed to update Nasal progress",
    });
  }
});
