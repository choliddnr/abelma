...
const calculateScore = () => {
  ...
  // Safety checks
  if (targetPixels === 0 || userPixels === 0) return { score: 0, stars: 0, coverage: 0, accuracy: 0 }
  ...
  return { score, stars, coverage: Math.round(coverage * 100), accuracy: Math.round(accuracy * 100) }
}
...
<template>
  <div class="relative w-full h-full min-h-[500px] bg-white rounded-3xl flex items-center justify-center shadow-lg border-2 border-gray-100 overflow-hidden" ref="containerRef">
    <!-- Dashed Letter Background -->
    <svg class="absolute inset-0 w-full h-full pointer-events-none select-none" xmlns="http://www.w3.org/2000/svg">
      <text 
        x="50%" 
        y="50%" 
