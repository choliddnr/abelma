...
import { useAlphabetStore, useRewardStore, useSyncStore } from '@/stores'
...
const challengeConfig = computed(() =>
  (alphabetProgress.challengeConfig && alphabetProgress.challengeConfig.length > 0)
    ? alphabetProgress.challengeConfig
    : DEFAULT_CHALLENGE_CONFIG
)
const currentConfig = computed(() => {
  const index = Math.max(0, level.value - 1)
  return challengeConfig.value[index] || challengeConfig.value[challengeConfig.value.length - 1] || DEFAULT_CHALLENGE_CONFIG[0]!
})
...
const launchConfetti = () => {
  ...
  const frame = () => {
    confetti({ particleCount: 5, angle: 60, spread: 55, origin: { x: 0 }, colors: ['#FFD93D', '#6BCB77', '#4D96FF', '#ff9a9a', '#A084E8'], zIndex: 100 })
    confetti({ particleCount: 5, angle: 120, spread: 55, origin: { x: 1 }, colors: ['#FFD93D', '#6BCB77', '#4D96FF', '#ff9a9a', '#A084E8'], zIndex: 100 })
...
const handleLetterClick = async (letter: string) => {
  if (speaking.value || isProcessingClick.value || showRewardCelebration.value || isChallengeCompleted.value) return
  ...
  if (letter === currentTarget) {
    ...
    setTimeout(() => { if (wrongLetter.value === letter) wrongLetter.value = '' }, 500)
  }
  isProcessingClick.value = false
}
  ...
  <div class="flex flex-col gap-4 animate-entrance">
    ...
    <div class="shrink-0 px-4 flex flex-col items-center justify-center min-h-[80px]">
      <div class="w-full max-w-4xl mx-auto flex flex-col gap-4">
        <div class="bg-white/30 backdrop-blur-lg p-4 md:p-6 rounded-4xl border-4 border-white/50 shadow-2xl relative overflow-hidden">
          <div class="absolute -top-10 -right-10 w-32 h-32 bg-indigo-400/20 rounded-full blur-3xl"></div>
          <div class="absolute -bottom-10 -left-10 w-32 h-32 bg-purple-400/20 rounded-full blur-3xl"></div>
          
          <div class="flex flex-wrap items-center justify-between gap-3 md:gap-6 relative z-10 w-full px-2">
            <!-- Progress -->
            <div class="relative w-14 h-14 md:w-16 md:h-16 flex items-center justify-center shrink-0">
              <div class="absolute inset-0 bg-indigo-500/10 rounded-full blur-xl animate-pulse"></div>
              <svg class="w-full h-full transform -rotate-90 drop-shadow-sm" viewBox="0 0 80 80">
                <circle cx="40" cy="40" r="34" class="text-indigo-100/50" stroke-width="8" stroke="currentColor" fill="transparent" />
                <circle cx="40" cy="40" r="34" class="text-indigo-500 transition-all duration-1000 ease-out" stroke-width="8" stroke-linecap="round" stroke="currentColor" fill="transparent" 
                  :stroke-dasharray="213.63" :stroke-dashoffset="213.63 * (1 - progressPercentage / 100)" />
              </svg>
              <div class="absolute flex flex-col items-center justify-center leading-tight">
                <span class="text-[10px] md:text-sm font-black text-indigo-600">{{ Math.round(progressPercentage) }}%</span>
              </div>
            </div>

            <!-- Stats -->
            <div class="flex-1 flex flex-wrap items-center justify-center gap-2 md:gap-4">
              <div v-if="currentConfig.timer > 0" class="ui-capsule bg-rose-50 border-rose-200 px-3 md:px-4 h-10 md:h-12 transition-all duration-300" :class="{ 'animate-pulse bg-rose-100 border-rose-400 shadow-lg': timeLeft <= 3 }">
                <span class="text-base md:text-xl animate-bounce">⏱️</span>
                <span class="text-base md:text-xl font-black text-rose-600 ml-1">{{ timeLeft }}s</span>
              </div>
              <div v-if="streak >= 2" class="ui-capsule bg-orange-50 border-orange-200 px-3 md:px-4 h-10 md:h-12 animate-float">
                <span class="text-base md:text-xl">🔥</span>
                <span class="text-base md:text-xl font-black text-orange-600 ml-1">{{ streak }}</span>
              </div>
              <div v-if="currentConfig.streak > 0" class="ui-capsule bg-amber-50 border-amber-200 px-3 md:px-4 h-10 md:h-12 border-dashed">
                <span class="text-xs md:text-sm font-black text-amber-700">Target: {{ currentConfig.streak }} 🔥</span>
              </div>
            ...
            <!-- Stop -->
            <div class="shrink-0">
              <button @click="emit('stop-challenge')" class="ui-capsule-interactive bg-rose-500 border-rose-600 text-white w-10 md:w-12 h-10 md:h-12 p-0 flex items-center justify-center shadow-lg hover:bg-rose-400 active:scale-95 transition-all">
    ...
    <div class="shrink-0 text-center h-14 md:h-16 flex items-center justify-center my-2 px-4">
      <transition name="fade" mode="out-in">
        <div v-if="feedback.message" :key="feedback.message" class="text-xl md:text-2xl lg:text-3xl font-black drop-shadow-sm animate-bounce"
          :class="{ 'text-secondary': feedback.type === 'success', 'text-danger': feedback.type === 'error', 'text-slate-600': feedback.type === null }">
          {{ feedback.message }}
        </div>
      </transition>
    </div>
    ...
    <div class="flex-1 px-4 pb-12 w-full max-w-7xl mx-auto overflow-visible relative">
      <div class="grid grid-cols-[repeat(auto-fit,minmax(70px,1fr))] sm:grid-cols-[repeat(auto-fit,minmax(90px,1fr))] md:grid-cols-[repeat(auto-fit,minmax(110px,1fr))] gap-3 sm:gap-4 lg:gap-5 w-full place-content-center">
        <BubbleCard v-for="(letter, index) in challengeLetters" :key="letter"
          as="button" @click="handleLetterClick(letter)"
          class="group cursor-pointer w-full aspect-square border-none rounded-[20%] sm:rounded-3xl animate-entrance"
          :class="[ getLetterColor(letter), wrongLetter === letter ? 'shake-animation bg-red-600 ring-4 ring-red-800' : 'hover:-translate-y-2 hover:shadow-xl shadow-md' ]"
      ...
      <div class="fixed bottom-6 right-6 md:bottom-10 md:right-10 z-20 animate-pulse">
        <button @click="playLetterSound(targetLetter)" class="flex items-center justify-center w-16 h-16 md:w-20 md:h-20 bg-white rounded-full shadow-2xl border-[5px] border-indigo-500 hover:scale-110 active:scale-95 transition-all">
    ...
    <transition name="pop-up">
      <div v-if="showRewardCelebration" class="fixed inset-0 z-100 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
        <div class="bg-white rounded-5xl p-8 md:p-12 shadow-[0_20px_50px_rgba(0,0,0,0.3)] border-8 border-yellow-400 max-w-sm w-full text-center relative animate-pop overflow-hidden">
          ...
          <h2 class="text-3xl md:text-4xl font-black text-yellow-600 mb-2 uppercase tracking-tight">Luar Biasa!</h2>
          <p class="text-xl md:text-2xl font-bold text-slate-600 mb-6">Kamu dapat Bonus Beruntun!</p>
          <div class="inline-flex items-center justify-center gap-3 bg-yellow-100 px-8 py-4 rounded-3xl border-4 border-yellow-200 shadow-inner">
            <span class="text-4xl md:text-5xl">🪙</span>
            <span class="text-4xl md:text-5xl font-black text-yellow-700">+{{ lastEarnedReward }}</span>
          </div>
          <div class="mt-8 text-sm font-black text-slate-400 uppercase tracking-widest animate-pulse">Koin Berhasil Ditambah!</div>
        </div>
      </div>
    </transition>
  </div>
...
.animate-entrance { animation: entrance 0.5s ease-out forwards; }
@keyframes entrance { from { opacity: 0; transform: translateY(20px); } to { opacity: 1; transform: translateY(0); } }

.pop-up-enter-active { animation: pop-in 0.5s cubic-bezier(0.34, 1.56, 0.64, 1); }
.pop-up-leave-active { animation: pop-in 0.3s cubic-bezier(0.34, 1.56, 0.64, 1) reverse; }
