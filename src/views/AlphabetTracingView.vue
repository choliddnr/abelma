...
const onSaved = () => {
  if (tracingCanvasRef.value) {
    ...
    if (result.stars === 5) {
      ...
      rewardStore.addPoints(15)
    } else if (result.stars === 4) {
      coinsEarned.value = 10
      rewardStore.addPoints(10)
    } else if (result.stars === 3) {
      coinsEarned.value = 5
      rewardStore.addPoints(5)
    } else if (result.stars === 2) {
      coinsEarned.value = 2
      rewardStore.addPoints(2)
    } else {
      coinsEarned.value = 0
    }
...
<template>
  <div class="flex flex-col items-center justify-center min-h-[80vh] w-full max-w-6xl mx-auto px-4 py-8 gap-8">
    <div class="w-full flex justify-between items-center">
      <button @click="goBack" class="btn-bubble bg-white px-6 py-3 text-xl font-bold text-gray-700 border-2 shadow-sm focus:outline-none">
        ← Kembali
      </button>
      
      <h1 class="text-4xl font-black text-blue-600 tracking-wide text-center bg-white/80 px-8 py-3 rounded-full shadow-sm">
    ...
    <!-- Canvas Container -->
    <div class="w-full h-full grow relative bg-white/50 backdrop-blur-md rounded-[40px] p-6 shadow-xl border-4 border-white">
      <TracingCanvas 
    ...
    <div v-if="showResult" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div class="bg-white rounded-[40px] p-8 max-w-lg w-full shadow-2xl border-8 border-blue-400 flex flex-col items-center text-center animate-bounce-in">
        ...
        <h2 class="text-4xl font-black text-blue-600 mb-6 drop-shadow-sm">
          {{ scoreResult.stars === 5 ? 'Sempurna!' : scoreResult.stars === 4 ? 'Luar Biasa!' : scoreResult.stars === 3 ? 'Bagus Sekali!' : scoreResult.stars === 2 ? 'Cukup Bagus' : 'Ayo Coba Lagi!' }}
        </h2>
        
        <div class="flex gap-4 text-6xl mb-6">
          <span v-for="i in 5" :key="i" class="transition-all duration-500 transform" :class="i <= scoreResult.stars ? 'text-yellow-400 scale-110 drop-shadow-lg' : 'text-gray-300 scale-90 grayscale'">⭐</span>
        </div>
        
        <div v-if="coinsEarned > 0" class="bg-yellow-100 border-4 border-yellow-400 text-yellow-700 px-6 py-3 rounded-full text-2xl font-bold mb-8 flex items-center gap-2">
        ...
        <div class="flex gap-4 w-full">
          <button @click="retry" class="flex-1 btn-bubble bg-gray-100 text-gray-700 font-bold text-xl py-4 border-4 border-gray-300">
            Ulangi
          </button>
    ...
    <transition name="toast">
      <div v-if="showIncompleteToast" class="fixed bottom-12 left-1/2 -translate-x-1/2 z-50 bg-red-500 border-4 border-red-300 text-white px-8 py-4 rounded-full font-bold text-2xl shadow-2xl flex items-center gap-4">
