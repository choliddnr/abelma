...
import { allAvailableStickers, earnedStickers } from '@/utils/stickerStore'
  ...
  <div class="min-h-screen bg-slate-50 flex flex-col p-4 md:p-8">
    ...
    <div class="flex-1 w-full max-w-5xl mx-auto">
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
            <div v-for="sticker in allAvailableStickers" :key="sticker.id"
                ...
                :class="earnedStickers.has(sticker.id) ? 'bg-white border-amber-300 shadow-xl' : 'bg-slate-100 border-slate-200 opacity-40 grayscale'">
                
                <!-- Emoji Display -->
                <div class="text-6xl md:text-8xl mb-4 drop-shadow-md transition-transform duration-500"
                    :class="earnedStickers.has(sticker.id) ? 'hover:scale-110 active:rotate-12' : ''">
                    {{ sticker.emoji }}
                </div>
                ...
                <div class="text-center">
                    ...
                    <p v-if="!earnedStickers.has(sticker.id)" class="text-xs text-slate-500 font-bold mt-1">
                ...
                <div v-if="earnedStickers.has(sticker.id)" class="absolute top-0 inset-x-0 h-1/2 bg-linear-to-b from-white/40 to-transparent rounded-t-[inherit] pointer-events-none"></div>
            </div>
        </div>
    </div>
